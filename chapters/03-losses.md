...
在大语言模型（LLM）预训练中，优化目标通常统一为自回归 Next-Token 交叉熵；图像分类任务亦主要依赖交叉熵损失。

然而在图像复原与超分辨率领域，损失函数通常呈现为多目标复合形式：
...
1. **不适定逆问题依赖多重先验约束**：每个独立的损失项均对应了一种物理或统计先验的数学表征
2. **不同损失函数主导不同的频段能量**：L2/L1 主导低频基底与结构保真，感知损失覆盖中高频语义纹理，对抗与扩散损失主导高频随机微观结构
3. **单一损失函数必然导致维度退化**：纯像素 L2 导致画面模糊，纯对抗损失引发不可控的幻觉伪影，纯高层感知损失易导致全局色调漂移

从先验映射视角审视：像素级 L1/L2 与 TV 属于**解析先验**（约束像素接近真值且局部平滑）；VGG/CLIP 特征距离属于**数据驱动先验**（约束深度特征流形对齐）；对抗与扩散损失属于**生成式先验**（强制解落在自然图像的真实分布曲面上）。模型最终的生成质量完全取决于各类先验梯度的动态制衡。
