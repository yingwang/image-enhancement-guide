...
- **SUREAL**：Netflix 开源的主观质量分数恢复工具库，实现的是最大似然主观模型，把评分人的偏差与不一致性一并估计出来（其主观分恢复方法源自 Li & Bampis 2017）
...
ITU-R BT.500-14（视频/影像主观评测的事实标准）在附录里给了一套基于 **β2（峰度）** 的异常评分人检测。它的逻辑不是"β2 大就可疑"，而是先用 β2 判断每张图的评分是否近似正态，据此选择不同的离群门限：

```
对每张测试图 j：
1. 算这张图所有评分人分数的 β2（峰度估计）
2. 若 β2 落在 [2, 4]，视该图评分为正态分布, 离群门限取 mean ± 2σ；
   否则视为非正态, 门限放宽到 mean ± √20·σ
对每个评分人 i：
3. 统计他有多少次分数落在对应图的门限之外
   (分别记高于上界的次数 P 和低于下界的次数 Q)
4. 若 (P + Q) / 总次数 > 0.05, 且 |P − Q| / (P + Q) < 0.3
   (说明是系统性两侧离群, 而非单向偏严/偏松), 剔除该评分人
```

关键在第 2 步：BT.500 是用 β2 是否落在 [2, 4] 来判正态、进而决定门限是 ±2σ 还是 ±√20·σ，而不是简单地"|β2| > 2 就标记可疑"。这套筛人方法比"剔除 catch trial 没过的"更系统，**正经发表论文/做产品评测应该用**。

工程实现：参考 [VQEG 的 SUREAL 库](https://github.com/Netflix/sureal) 里 `bt500.py` 的算法实现。

## 12.7 SUREAL：现代 MOS 估计

简单的"对每张图取所有评分人的均值"假设了所有评分人同等可信。Netflix 开源的 **SUREAL** 工具库（主观模型的最大似然实现，主观分恢复方法源自 Li & Bampis 2017）把 MOS 建模为：
...
- **审美差异**：东亚评分人更倾向认为"过度锐化 + 高对比"是"好画质"，欧美评分人更倾向"自然 + 低伪影"，同一张图两群人给的 MOS 可能有可观的差距
