...
### 2024-2026：从回归均值到生成式视频扩散复原

回归式模型（如 BasicVSR++、RVRT）在数学本质上逼近像素均值（基于 $\ell_1 / \ell_2$ 损失优化）。面对严重退化或需要深度幻觉补全细节的真实视频时，回归式输出倾向于表现出偏软、保守的平滑伪影。2024 年起，视频增强前沿显著转向以视频扩散模型（Video Diffusion）为强先验的生成式复原路线，将图像端扩散超分辨率的成功经验拓展至时空维度：

- **Upscale-A-Video（CVPR 2024）**：将图像潜空间扩散先验拓展至视频序列，引入时序卷积层与光流引导约束维持帧间连贯性；
- **MGLD-VSR（ECCV 2024）**：在潜空间中基于运动引导的扩散过程执行真实世界视频超分辨率；
- **VEnhancer（2024）**：利用统一的生成式视频扩散网络联合完成视频超分辨率与插帧增强；
- **STAR 与 SeedVR / SeedVR2（2025-2026）**：将文生视频扩散模型的世界先验引入真实世界 VSR，SeedVR2 进一步推进单步扩散推理（One-step Generation），显著压缩了多步去噪的端到端延迟。

生成式视频扩散先验在打破回归均值模糊的同时，也带来了两项核心工程代价：
1. **时序一致性风险**：扩散先验的生成自由度更高，若时序约束不严，帧间纹理沸腾与物体身份漂移会更加显著，需配合强物理光流引导与潜空间特征传播；
2. **推理延迟与算力开销**：多步扩散采样叠加上时间轴维度，推理开销较 BasicVSR++ 高出一到两个数量级，目前主要服务于离线转码与影视修复。逐步迈向单步化推理（如 SeedVR2）是将扩散视频复原推向工业实时的关键技术方向。
