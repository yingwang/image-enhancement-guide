...
> 这是初涉视频增强领域最常见的认知误区。
>
> 视频增强的核心挑战在于时序一致性：即便单帧输出在客观指标上表现优异，若缺乏帧间约束，连续播放时仍会出现严重的抖动、闪烁与纹理沸腾。

## 13.0 阅读须知

前十二章系统阐述了单帧图像增强的技术体系：退化建模、特征表示、损失设计、训练稳定化与评估方法。从本章起进入全书 Part IV，核心主题拓展至**时序视频维度**。视频在数据结构上表现为带有时间维度的图像序列 $(T, C, H, W)$，但时间轴引入了全新的时序动力学与运动约束。本章与第 14 章将系统解构逐帧推理失效的物理根源，并建立面向视频增强的工程对策。

阅读前置依赖：读者需掌握第 1 至 3 章的退化与损失基础、第 6 至 10 章的图像模型架构，以及第 12 章的评估方法论。本章不预设读者具备视频编解码或光流估计的先验经验，涉及的核心概念均在首次出现时给出规范定义。

读完本章，你应当能够掌握：逐帧独立增强引发时序闪烁的机理；如何基于光流场构建可微的时序一致性损失；视频压缩与运动模糊等专属退化的合成方法；滑动窗口、单向循环与双向循环三大时序建模范式的工程取舍；以及针对视频画质的时序评估指标（tOF、tLPIPS）与流水线优化方案。
...
选取一段 720P 分辨率的手机录制视频，使用预训练的 Real-ESRGAN 模型对每一帧独立执行 4 倍超分辨率重建，放大至 4K 分辨率。对比观察会呈现显著反差：

- **单帧截屏静态观察**：每一帧的细节纹理丰富、边缘锐利，单帧主观评分（MOS）可从 2.5 提升至 4.0；
- **连续动态播放观察**：画面出现严重的边缘高频抖动、纹理闪烁以及微观结构漂移，动态主观体验骤降至 2.0 以下。

高频出现的时序失真现象包括：

1. **结构性"呼吸"效应**：静态背景中的边缘（如墙壁线条或静止物体轮廓），在各帧生成的微观亚像素结构存在差异，连续播放时呈现类似缓慢扩张与收缩的呼吸感；
2. **纹理"沸腾"伪影**：镜头平移或物体匀速移动时，物体表面的高频纹理无法随运动轨迹平滑迁移，而是在局部随机重构，产生类似液体沸腾的视觉干扰；
3. **平坦区域噪点闪烁**：天空、纯色墙面等低频平坦区域因单帧高频生成先验的微小扰动，产生明暗跳变的闪烁斑点；
4. **文本笔画抖动**：微小字符笔画边缘在相邻帧间发生亚像素级位置跳变，产生抖动感。
...
视频增强算法的演进路径本质上围绕这一权衡展开：早期方案（2015 至 2017 年）直接套用图像模型；中期方案（2017 至 2020 年）引入显式光流与运动补偿 Warping 模块；现代主流方案（2021 年至今）则通过双向循环网络（如 BasicVSR++）与时空注意力机制（如 VRT/RVRT），将时序信息隐式编码进特征传播流。
