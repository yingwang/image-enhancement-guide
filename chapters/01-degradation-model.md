...
- $\hat{x}$：模型对 $x$ 的估计
- $D$：退化算子（degradation operator），一个把高质量图像变成低质量图像的过程（本书个别章节会在局部另行借用同一个字母表示别的对象，例如第 3 章的判别器也记作 $D$，届时会在该处显式说明，不与这里的退化算子混淆）
- $n$：噪声
...
具体压成什么？取最简单的一种下采样，box（区域均值）下采样：把每个 $4 \times 4$ 的高分辨率块直接做算术平均，得到 $y$ 里的一个像素。这是一个 16 维空间到 1 维空间的映射。**16 维降到 1 维，丢掉 15 维信息**。（工程上更常用的 bicubic 下采样并不是这样：它的插值核支撑区大于 $4 \times 4$、而且带负瓣，并不把某个 $4 \times 4$ 块严格压成一个值。这里改用 box 均值举例，只是为了把"丢掉多少维"这笔账算干净，至于维度坍缩、信息不可逆丢失这一点，两种下采样是一致的。）
...
所有这些不同的高分辨率块经过下采样都给你同一个 128。模型必须**选一个**输出，到底选哪一个？
...
这一类的特点：**判别式（discriminative，意为给定输入直接输出最优答案）**，给一个 $y$ 就输出一个 $\hat{x}$。模型不显式建模 $p(x)$，但在大数据集上训练后，输出的 $\hat{x}$ 自然落在自然图像流形上。
...
视频压缩（H.264/H.265/AV1）类似但更复杂，还多了运动补偿带来的拖影。
...
- **去运动模糊**：相机记录了曝光期间的 IMU 数据 → 模糊核 $k$ 可解析
- **医疗 CT 重建**：扫描仪几何 + 投影几何 → 系统矩阵 $A$ 已知
- **去摩尔纹**：屏幕拍摄已知屏幕分辨率 → 采样 aliasing 模型已知
- **传感器去噪**：传感器型号 + ISO + 曝光 → 噪声 PDF（高斯 + 泊松 + 暗电流）参数已知

非盲场景可以走经典逆问题路线：Wiener 滤波、Richardson-Lucy、迭代正则化（TV / sparse / plug-and-play）。深度学习也可以非盲：把 $D$ 的参数（模糊核、噪声水平）作为额外输入喂给网络（DnCNN-Blind、KernelGAN 等）。

### Blind：D 未知

真实世界增强里 $D$ 几乎永远未知：

- 用户从手机相册扔进来一张图，谁知道它经历过什么
- 网络上下载的图被压缩了几次
- 老照片的退化是物理化学过程，没有 closed-form 模型
- 屏幕截图叠加了渲染、缩放、JPEG

Blind 场景下你**没法精确知道 $D$**，只能做两件事：

1. **训练时让模型见过尽可能广的 $D$ 分布**（Real-ESRGAN 的二阶退化 pipeline 就是这件事，第 5 章详谈）
2. **运行时让模型隐式估计 $D$**（KernelGAN 估核、SRMD 把退化参数当条件、扩散派的 prompt 里带 LLaVA 描述）

### 为什么这个区分重要

这本书后面讲的所有 SOTA 模型在 blind/non-blind 上的位置：

| 模型 | 范式 | $D$ 处理方式 |
|------|------|------------|
| Real-ESRGAN | **Blind SR** | 训练时见过百种合成 $D$ |
| CodeFormer | **Blind face restoration** | codebook 先验给"人脸应当长什么样"，绕开估 $D$ |
| BSRGAN | **Blind SR** | 与 Real-ESRGAN 同期，退化合成路线 |
| MANIQA / CLIP-IQA / Q-Align | **Blind IQA**（NR-IQA） | 没有参考图，直接打分 |
| SUPIR | **Blind**（含 LLaVA prompt） | 用文本条件隐式补充语义 $D$ |
| Restormer | **可 blind 可 non-blind** | 训练数据决定，结构上不依赖 $D$ |
| OSEDiff / TSD-SR | **Blind SR** | 同 SUPIR |

工程结论：**生产环境里凡是面向"用户传图"的产品，几乎只能做 blind**。任何依赖 $D$ 已知的算法（哪怕在 benchmark 上漂亮）落到产品上都得做"$D$ 估计"前置，而估计本身又是一个 ill-posed 问题，估错就崩。

序章承诺的"看到一张烂图就能判断"，本质就是 blind 范式的口语化：读者训练的是"在 $D$ 未知时如何选先验"。这条线贯穿全书。
...
- EDSR、RCAN 这类 PSNR 导向的模型在 Set5/Set14（学术 benchmark）上能到 PSNR 30+，数字很漂亮
- 同样这批模型（连同为了锐度主动牺牲几分贝 PSNR 的 ESRGAN）搬到你手机里的真实照片上，几乎都不工作；ESRGAN 尤其明显，会把噪点放大成一层糖纸似的伪影
