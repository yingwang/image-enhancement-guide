...
模型并非在物理意义上“找回了原本的真实场景”：从信息论角度来看，原始高频信号在光电转换、弥散滤波与离散量化的物理退化过程中已经发生了不可逆的信息坍缩。模型所做的，是**依据当前的退化观测，在所有可能的高清图像候选集构成的解空间中，搜索并估计出一张最符合自然图像先验分布的合理构型**。这是贯穿全书的核心认知支点：

> 增强模型并非在严格意义上逆转物理熵增去“复原”真实，而是在不确定性约束下做出最优“推定”。算法工程的优劣，取决于推定结果在多大程度上契合自然图像分布流形与物理成像规律。

确立了这一认知，后续关于网络架构、损失函数、评估指标与退化合成的所有设计选择，便拥有了统一的方法论支点。

## 1.2 中心方程

将上述物理退化与观测过程形式化为数学表达式：
...
- $D$：退化算子（由光学系统衍射、透镜弥散、传感器重采样、色彩映射与有损编码共同构成的复合映射）
- $n$：噪声项（源于光子到达的统计涨落与传感器电路的读出扰动）
- $y$：实际观测到的低质退化图像
...
1. $D$ 通常**未知**（拍摄时的镜头像差、传感器温度特性、传输中的压缩量化表等物理参数均无法精确获知）
2. $D$ 具有**多对一**的非单射性（由于频域截止与空间下采样，无数张不同细节的高清图 $x$ 经过 $D$ 投影后都会坍缩至相同的观测 $y$）
3. $n$ 具有**随机性**（同一物理场景在相同条件下连续曝光两次，光子涨落的随机采样也会导致观测 $y$ 存在统计差异）
4. 训练数据中通常**仅有 $y$**（真实业务场景中极难采集到严格物理对齐的退化-高清成对基准）
...
1. **解存在**
2. **解唯一**
3. **解对输入具有连续依赖性**（输入的小扰动仅引起输出的小扰动，系统具备数值稳定性）
...
以最简化的区域均值（box filter）下采样为例：每个 $4 \times 4$ 的高清像素块直接取算术平均，映射为低分辨率图像中的一个像素。这构成了从 16 维向量空间到 1 维标量空间的投影，**维度从 16 骤降至 1，丢失了 15 个维度的正交自由度**。（工业界更常用的双三次插值 bicubic 下采样虽然插值核支撑域更大且包含负瓣，并非严格的块内均值，但在维度坍缩与高频信息不可逆损失的数学本质上完全一致。）

反向推导：给定低分辨率像素标量 $y_{ij}$，能够生成该观测的 $4 \times 4$ 高分辨率图像块构成了一个 15 维的零空间（null space），其中包含无穷多个可能的候选块。
...
所有这些在结构上迥异的高分辨率块，经过下采样算子后均映射为单一数值 128。当算子 $D$ 抹去零空间内的所有高频分量后，仅凭观测 $y$ 无法判定哪一种纹理对应真实场景。模型必须在解空间中选定一个输出，这一决策完全依赖先验（prior）。
...
2014 年 **SRCNN**（Super-Resolution Convolutional Neural Network，三层卷积超分辨率的奠基性工作）问世后，深度学习全面取代了传统变分方法。卷积神经网络构建了从低质图像 $y$ 到高清图像 $\hat{x}$ 的端到端映射，图像先验被隐式编码在网络权重参数中。模型在海量自然图像对 $(x, y)$ 上进行优化，学习到的是自然图像在高维参数空间中的低维流形拓扑结构。
...
在离散图像处理中，空间不变模糊等价于理想图像信号与光学系统的点扩散函数（Point Spread Function, PSF，即“模糊核”）$k$ 的二维空间卷积：$y = x * k$。

不同物理诱因对应不同形态与频域特性的模糊核：

- **散焦模糊**（defocus blur）：模糊核近似于均匀圆盘函数（Airy 斑的几何光学近似），其弥散半径由失焦物距、镜头焦距与光圈孔径共同决定
- **运动模糊**（motion blur）：模糊核呈现为带有方向与长度的线段或轨迹曲线，表征曝光时间内相机机身或目标物体的相对位移积分
- **大气湍流模糊**（atmospheric turbulence）：模糊核近似于各向同性或各向异性高斯函数，方差由折射率结构常数与湍流积分强度决定
- **光学系统像差与衍射**：模糊核表现为复杂的非对称波前畸变斑纹

实际成片往往是多种模糊形态的物理叠加，且普遍呈现**空间可变性**（视场中心与边缘透镜边缘处的像差核存在显著差异）。核参数的未知性促成了**盲去模糊**（Blind Deblurring）作为独立研究方向的存在。

### 下采样（Downsample）

将连续空间光学信号离散化为低空间分辨率的网格。工程常用算子包括：

- **Nearest（最近邻插值）**：直接采样最近网格点，计算开销极低，但由于频谱未滤除即发生采样，会引入严重的阶梯状高频混叠（锯齿）
- **Bilinear（双线性插值）**：基于 $2 \times 2$ 邻域进行一阶张量积线性加权插值
- **Bicubic（双三次插值）**：基于 $4 \times 4$ 邻域三次样条核卷积，为学术基准数据集**事实上的标准下采样基准**
- **Lanczos**：基于加窗 sinc 函数的截断滤波插值，在频域通带内平坦度最优，带外滚降陡峭
...
在重采样行为的频域响应对比中：双三次插值保留了更丰富的过渡斜率但易诱发边缘过冲；区域均值插值兼顾局域积分能量守恒但抑制了高频响应；最近邻插值则因高频折叠直接引入阶梯状混叠伪影。不同离散重采样算子在数学本质上对应着不同的连续函数重构核与频域带通滤波器。
...
- **医学 CT 图像重建**：X 射线探测器几何拓扑与射线投影路径已知，系统投影矩阵 $A$ 精确可求
