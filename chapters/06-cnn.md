...
> 2014 年至 2022 年间，卷积神经网络在低层视觉领域走过了一条清晰而深邃的演进轨迹：
>
> 从模仿稀疏编码的浅层映射（SRCNN），到深层堆叠与残差学习（VDSR/EDSR）；从通道维度的自适应重加权（RCAN），到密集连接的多级嵌套复用（RRDB）；再到去繁就简、剔除传统激活的门控精炼（NAFNet）。
>
> 这八年的技术积淀，构筑了现代图像增强领域的底层工程基石。理解这些经典架构在感受野、特征复用与数值优化上的取舍哲学，是洞悉后续视觉 Transformer 与扩散模型演进脉络的前提。
...
自 2021 年起，SwinIR、Restormer、HAT 等 Transformer 架构在低层视觉领域大放异彩；2023 年后，StableSR、SUPIR 等扩散模型更是在生成式图像超分上刷新了视觉上限。表面上看，CNN 似乎已成往事。

**然而在工程实践中，CNN 依旧占据着坚不可摧的基石地位。** 几个关键事实不容忽视：
...
**SRCNN 的核心历史价值不在于其绝对重建质量，而在于证实了端到端深度学习在图像超分辨率任务中的可行性。** 在 SRCNN 诞生之前，传统超分辨率算法普遍依赖分步级联的复杂流水线：图像块特征提取、稀疏字典编码、高维特征映射与高分辨率图像重建。各阶段参数独立优化且不可微，无法实现全局梯度的协同演进。SRCNN 将这一复杂流水线完全映射为三层紧凑的卷积神经网络，仅凭标准随机梯度下降即可实现全局联合优化，奠定了后续深度超分辨率研究的基调。
