...
> 前十章系统构建了影像增强的理论基石：模型架构、损失函数与退化合成。
>
> 本章聚焦工程落地的中枢命题：如何将这些模块熔铸为稳健高效的训练流水线。
>
> 相比分类或大语言模型，低层视觉模型的训练动力学更为敏感脆弱：多目标损失相互牵制、对抗博弈起伏动荡、扩散时间步调度高度非平稳，微小的配置失调便可能导致数天训练付诸东流。

## 11.0 阅读须知

本章承接全书前面的理论与方法体系。第 1 至 2 章建立了问题定义与物理成像退化模型，第 3 章梳理了损失函数谱系，第 4 章剖析了评估指标的测量边界，第 5 章构建了数据退化流水线，第 6 至 10 章则横跨了主流模型架构。将这些模块拼装成可运行的代码只是工程起点；真正的分水岭在于能否驾驭复杂的优化动力学，保障训练**连续数天乃至数周保持数值稳定**，并最终收敛至预期的性能高地。本章系统提炼低层视觉训练工程的实战经验。

假设读者已熟练掌握 PyTorch 基础训练循环（前向传播、反向传播、优化器步进与数据加载），但**未必具备低层视觉专项训练经验**。与图像分类或大语言模型相比，低层视觉的训练动力学在四个维度存在本质分野：

- **密集连续张量映射**：优化目标并非将整图压缩为离散类别标量，而是高维张量间精细到亚像素级别的保真重构；
- **复合损失的动态博弈**：极少依赖单一损失函数，通常由 3 至 5 项相互制约的损失加权驱动，权重配置的毫厘之差即可导致优化方向在保真度与感知纹理之间单向漂移；
- **非平稳分布与多步采样**：生成对抗网络（GAN）深陷生成器与判别器的零和对抗，扩散模型（Diffusion）则横跨全时间步的加噪与去噪调度，优化曲面的局部曲率起伏剧烈；
- **紧耦合的在线退化流水线**：为提升训练吞吐，退化合成通常流水线化部署于 GPU 端（详见第 5 章），其采样逻辑与概率分布的隐蔽瑕疵会直接导致模型拟合出人工合成伪影，严重侵蚀真实场景下的泛化能力。

**首次出现的缩写。** 为方便后续章节引用，本章涉及的关键缩写定义如下：

- **AMP**（Automatic Mixed Precision，自动混合精度）：前向传播与反向传播采用 FP16/BF16 计算，主权重副本与梯度累积保持 FP32 的混合精度训练范式
- **EMA**（Exponential Moving Average，指数移动平均）：在训练权重之外维护一份参数的时序滑动平均副本，推理与评测时使用 EMA 权重
...
大语言模型的自回归训练通常由单一交叉熵损失（Cross-Entropy）主导，优化曲面相对规整：在模型参数量、数据规模与学习率匹配的前提下，损失曲线往往呈现平稳单调的幂律收敛。低层视觉增强模型则身处完全不同的优化境地：

- **多目标帕累托前沿的拉锯**：L1 像素损失执着于绝对能量误差最小化，VGG 感知损失在深层特征空间丈量语义距离，对抗损失则逼迫网络生成符合真实数据分布的高频细节。任何一项权重的失衡都会导致优化轨迹滑向极端的局部次优：或陷入高 PSNR 却过度平滑的塑料感，或追求锐利边缘却诱发严重的结构性崩塌；
- **对抗博弈的非平稳动态**：判别器（D）与生成器（G）处于动态攻防之中。判别器过强会导致判别损失迅速归零、生成器梯度饱和消失；判别器过弱则无法提供有效的高频监督，使生成器丧失细节刻画能力；
- **扩散时序调度的敏感性**：时间步采样策略、信噪比损失加权（如 Min-SNR）以及 EMA 衰减系数深度交织，任一环节配置不当均会导致去噪轨迹偏离真实流形，降低生成样本的物理合理性；
- **退化流水线的隐蔽误差注入**：退化合成逻辑（详见第 5 章）通常包含数十个连续随机变量。若某项退化参数的先验分布与真实物理退化脱节，训练系统并不会抛出运行时异常，而是默默将模型引向错误的泛化方向。

本章将低层视觉工程中的高频隐患提炼为系统化的防御体系。建议将本章作为工程速查手册：搭建项目初期参照 11.2 的标准骨架，调优超参数时查阅 11.3 至 11.8，遭遇异常波动时检索 11.15 的故障诊断矩阵。
...
数据流中有两处关键工程决策值得深究：其一，**退化合成模块（B）在 GPU 端与 CPU DataLoader（A）端的部署权衡**。CPU 端合成具备极高的逻辑灵活性且易于多进程并行，但在高分辨率、小 batch 场景下，频繁的主机到设备数据传输极易导致 PCIe 带宽成为系统吞吐的死锁瓶颈；GPU 端在线合成直接规避了总线通信延迟且天然支持微分操作，但会占用宝贵的显存与 CUDA 计算核心。例如 Real-ESRGAN 的官方工程实现便全面采用了 GPU 端批处理合成方案。其二，**EMA 更新（I）必须严格位于优化器 step 之后**，且 EMA 副本参数全程脱离动态计算图，仅在验证评估与最终权重持久化时被加载激活。
...
上述取值源自大量 SOTA 论文与工业级开源库在充分收敛后的经验共识区间，其前提设定为 batch size 介于 16 至 32、总训练步数处于 200K 至 1M 且搭配 AdamW 与余弦退火。若调整批量大小或迭代周期，需遵循 11.5 节给出的缩放准则。

参数 `betas=(0.9, 0.99)` 的设置蕴含着明确的工程直觉。PyTorch 原生 Adam 默认 $\beta_2 = 0.999$，对应较长的二阶矩指数滑动估计窗口，适合单目标凸优化；但在低层视觉的多损失与对抗训练中，损失曲面的局部曲率剧烈起伏，过长的历史滑动窗口会导致二阶矩估计严重滞后于瞬时梯度突变，在训练中后期极易诱发损失尖刺。将 $\beta_2$ 调至 $0.99$ 能够显著加快二阶矩对梯度瞬态变化的追踪响应速度，增强复杂非凸地形下的数值稳定性。

## 11.4 学习率调度

### Linear Warmup

训练初期将学习率从接近零线性爬升至目标峰值。在 Transformer 与扩散模型训练中属于**必备组件**，能够防止初始化权重产生的大梯度在初始阶段破坏参数流形的拓扑结构：
...
在长期训练阶段周期性将学习率重置至较高峰值，辅助模型跳出局部鞍点：
...
工程经验：在长周期扩散模型训练中，余弦重启能够持续促进微观细节质量的渐进提升。
...
2. 调用退化合成流水线（详见第 5 章），对 HR Patch 执行下采样、模糊与加噪，生成对应的 LR Patch；
...
1. **显存物理边界**：单张 $2048 \times 2048$ 的 3 通道 FP32 图像占用约 50MB 内存，叠加 Batch 维度与网络深层激活后，单卡显存无法承载全图直接反向传播；
2. **空间数据多样性**：随机裁剪充当了隐式的空间正则化，使网络在不同 Epoch 中覆盖多样化的局部纹理组合；
3. **计算吞吐增益**：在固定 GPU 算力开销下，Patch 化能够以更大的 Batch Size 遍历更多场景样本；
4. **张量规整对齐**：原始数据集中图像尺寸各异，裁剪为固定 Patch 能保证 DataLoader 高效批处理堆叠。
...
调整 Batch Size 时的常规参考规则包括平方根缩放（$\text{Batch} \times 2 \implies \text{LR} \times \sqrt{2}$）与线性缩放（$\text{Batch} \times 2 \implies \text{LR} \times 2$）。鉴于低层视觉对优化步长极为敏感，工程上建议以基线配置运行 5,000 步实测收敛斜率，再决定学习率的缩放系数，避免盲目线性放大导致发散。
