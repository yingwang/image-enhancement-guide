...
感知损失之所以具备卓越的感知度量能力，其物理机制在于：预训练卷积核在多层前向传播中自发形成了类似生物视觉皮层的感受野分工：

- **浅层特征（relu1_x, relu2_x）**：感受野极小，对局部高频边缘、角点及颜色梯度跃变高度敏感
- **中层特征（relu3_x, relu4_x）**：感受野适中，捕捉局部几何纹理、材质分布与结构部件，是感知损失的核心支撑层
- **深层特征（relu5_x）**：感受野覆盖全局，高度抽象为物体语义类别，对空间绝对平移与轻度几何形变具有极高容忍度

工程选层原则：过浅的特征损失易退化为带有滤波掩码的像素级 L1，丧失高阶结构约束；过深的特征损失则会导致模型忽略像素级的空间几何对齐，产生语义自洽但位置漂移的结构幻觉。

需要指出的是，经典感知损失亦存在其固有的技术边界：

- VGG-19 拓扑较为古老，特征空间对现代多模态大模型所需的对齐维度覆盖有限
- ImageNet 分类先验对自然物体高度敏感，但对文本排版、遥感地表或半导体显微等特定垂直领域的纹理保真度缺乏精细区分力
- 现代工程中通常结合 **LPIPS**（基于人类主观偏好校准的深度特征距离）与 **DISTS**（解耦结构与纹理的距离度量）进行联合监督（详见第 4 章）
