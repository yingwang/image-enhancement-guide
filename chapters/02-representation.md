...
graph LR
    ...
    Latent -->|VAE 解码器 Dec| Pixel2[像素空间<br/>解码后]
...
更具体的估计：研究界对自然图像的"有效维度"做过多个测量，结论大致在几百到几千维这个量级。也就是说，一张百万像素图的真实信息量大致相当于一个几千维的向量，其余几十万维都是高度相关的冗余。VAE 把 1024×1024 图压成 128×128×4 = 65536 维，这个体量比原始的三百多万像素少了近五十倍，说明潜空间编码确实在做"抹除冗余"的本职工作。但要留意一个尺度差：65536 维仍然远大于"几百到几千维"的内在维度估计，所以 VAE 潜表示并没有逼近图像的内在维数，它依旧是一种有损、且自身仍带冗余的编码，只是比像素空间的冗余少得多。
...
2020 年的 DDPM 论文在像素空间训练 $256 \times 256$ 的扩散模型。这里要分清训练与推理：训练时每一步只随机采一个时间步 $t$、做一次去噪预测，单步成本并不高；真正昂贵的是推理采样，要在 $256 \times 256 \times 3 = 196,608$ 维上反复去噪 1000 步才能生成一张图。把分辨率提到 $1024 \times 1024$，每一步的计算量直接翻到 16 倍：这就是为什么早期扩散模型都局限在 $256 \times 256$。
...
选哪几层做感知损失，本质就是选你希望模型"在哪个尺度上像真值"。取得太浅，损失接近像素 L1，丢掉了 VGG 的语义优势；取得太深，模型可以输出一张完全不像但语义正确的图（比如一只跟原图不同姿势的猫），损失仍然很小。**relu2_2 + relu3_3 + relu4_3 这三层加权**是 Johnson 等人 2016 年在风格迁移与感知损失里用的经典中层组合，上面那段示例代码取的也是这一组。这里要澄清一个常见误传：ESRGAN、Real-ESRGAN 的感知损失并不用这组浅中层，而是把重心放在更深的 conv3_4 / conv4_4 / conv5_4，并且取的是激活函数之前（pre-activation）的特征，理由是深层激活过于稀疏、激活前的特征能提供更密的监督。
...
- **解码器** $\mathrm{Dec}: \mathbb{R}^{h \times w \times c} \to \mathbb{R}^{H \times W \times 3}$（这里刻意把解码器记作 $\mathrm{Dec}$ 而不是 $D$，因为字母 $D$ 在第 1 章已经占给了退化算子、第 3 章又会占给判别器，避免一符多义）

训练目标是让 $\mathrm{Dec}(E(x)) \approx x$，且 $E(x)$ 服从一个简单的先验分布（通常接近高斯）。

这里多说一句"变分"的含义，便于没接触过 VAE 的工程师建立直觉。普通自编码器只要求 $\mathrm{Dec}(E(x)) \approx x$，对潜表示 $z = E(x)$ 的分布本身不做约束，因此潜空间往往是"破碎"的：训练样本各自占据潜空间里的一些孤岛，岛与岛之间的区域不可解释。VAE 多加一项 KL 散度约束，把所有训练样本的潜表示都拉到一个标准正态附近，让潜空间变成连续的、可采样的。代价是重建质量略有下降，收益是潜空间变成了一个可以做生成、可以做插值、可以加噪声的可控空间。
...
graph LR
    ...
    Z -->|解码器 Dec<br/>3 次反卷积| Xhat[重建图 x_hat<br/>512 x 512 x 3]
    Z -.->|加噪 / 扩散 / 编辑| ZPrime[扰动后 z']
    ZPrime -->|解码器 Dec| Xprime[新图 x']
...
更彻底的应对是**换 VAE 配置**。SDXL 用的 VAE 比 SD1.5 更深，潜空间通道数仍然是 4（并没有增加），但对 latent 的幅度（magnitude）做了更稳的校准，重建 PSNR 提升约 1-2 dB；FLUX 的 VAE 把潜通道升到 16，重建 PSNR 又提升若干 dB；近期一些工作（如 LiteVAE、HViT-VAE）干脆把 VAE 重新训练以专门服务增强任务。这条路径解决问题的根本是"提高 2% 信息可携带量"，代价是计算量与生态兼容性。如何在 VAE 重建上限、计算成本、生态兼容这三者之间权衡，是潜空间扩散派增强模型的工程主线之一。
