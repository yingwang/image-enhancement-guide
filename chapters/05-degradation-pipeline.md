...
在 2014 至 2020 年间的低层视觉研究中，曾长期存在一种方法论范式偏差：

1. 仅采用理想双三次下采样（Bicubic）在清晰图像上进行规则重采样，合成配对训练数据
2. 在受控合成测试集上迭代网络拓扑并刷新 PSNR/SSIM 榜单
3. 当部署于手机夜景实拍、社交网络重压缩或老旧胶片数字化等真实场景时，模型因输入分布严重失配而发生灾难性的结构崩塌与噪点放大

Real-ESRGAN（Wang et al. 2021）通过一组严密的控制变量实验，揭示了数据退化流形建模对最终画质的决定性作用：
...
实验表明：**在网络骨干拓扑保持完全一致的前提下，仅重构退化数据的合成流形，即可使模型在真实场景下的可用性实现跨越式蜕变。**

这一结论在图像去噪、去模糊与前沿扩散增强领域同样被反复验证：

- 经典 DnCNN 在合成高斯白噪声测试集上表现优异，但在面对真实传感器在暗光环境下的异方差非高斯噪声时迅速失效；仅当引入真实物理 RAW 域噪声建模后，网络才具备真实的降噪泛化能力。
- 前沿多模态扩散增强模型（如 SUPIR）之所以在开放域画质复原中展现出卓越的质感重建能力，其核心支柱并非复杂的提示词引导机制，而是底层依托千万级高质量图像与复杂高阶退化合成管线构建的坚实数据底座。
