...
在同一个测试集（若干张退化的猫脸图）上，分别用三个模型做 4× 超分，然后统计指标（PSNR、SSIM、LPIPS 在全集上取平均，FID 在整个输出集合与真值集合之间算一次）：
...
多尺度 SSIM：把图重复下采样几次，每次都算 SSIM，加权平均。优点：能捕捉不同尺度的结构相似度。在视频质量评估里 MS-SSIM 常常与 PSNR 一起报告，因为它对人眼感知质量的预测能力比单尺度 SSIM 强一些。对增强任务它也是一个不亏的"附加指标"：计算开销小，能补充 SSIM 单尺度的盲区。
...
> 在 ill-posed 逆问题上，**distortion**（失真，指 PSNR、SSIM 乃至 LPIPS、DISTS 这类需要与真值逐一配对比较的全参考指标）和 **perception**（感知，指 FID、KID 这类只衡量输出分布与自然图像分布有多接近、不需要配对的分布散度）**不可同时最优**。
>
> 任何优化算法都在这条 trade-off 曲线上找一个点。

这里有一个容易混淆的地方要点破：LPIPS 虽然常被口头叫作"感知指标"，但在 Blau-Michaeli 的框架里它属于 **distortion 一侧**。划线的标准不是"这个指标像不像人眼"，而是"它要不要跟真值逐一配对"。LPIPS、DISTS 都必须拿到成对的 $(x, \hat{x})$ 才能算，本质上仍是全参考失真度量，只不过是**感知友好的失真**（在特征空间而非像素空间量差异），因此它比 PSNR 更能奖励真实细节。真正落在 perception 轴上的，是 FID、KID 这种只比较 $\{\hat{x}\}$ 与 $\{x\}$ 两个分布、完全不需要配对的分布散度。理解这一点，才不会把"LPIPS 变好"直接等同于"到了 perception 端"。
...
graph LR
    ...
    C -.PSNR 低<br/>LPIPS 低.-> Score2[右端最优指标:<br/>FID / 分布散度]
